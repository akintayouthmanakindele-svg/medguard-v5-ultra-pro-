...
// ─── PDF Generator ────────────────────────────────────────────────
async function generatePDF(element, filename, title, timestamp, setLoading) {
  ...
  try {
    ...
    pdf.text(`${title} — ${timestamp}`, margin, 8);
    pdf.addImage(imgData, "PNG", margin, 14, imgW, Math.min(imgH, pageH - 24));
    pdf.save(filename);
  } catch(e) {
...
function NetworkBanner({ online }) {
  ...
  return (
    <div style={{
      ...
      📴 Offline — IV Calculator, Risk Scores, Drug Ref & Lab Values
      work without internet. Clinical evaluation resumes when you reconnect.
    </div>
  );
}
